...
def main_menu() -> None:
    ...
    image_db = dm.load_image_db(data_path)
    select_option: str = ''
    while select_option != '0':
        ...
        match select_option:
            case '1':
                settings['folders'] = manage_folders(settings['folders'])

            case '2':
                settings['extensions'] = manage_extensions(
                    settings['extensions'])
            case '3':
                image_db = dm.scan_folders(settings['folders'],
                                           settings['extensions'],
                                           image_db)
            case '4':
                image_process(image_db, in_place=True)
            case '5':
                dm.save_settings(settings_path, settings)
            case '6':
                dm.save_image_db(data_path, image_db)
